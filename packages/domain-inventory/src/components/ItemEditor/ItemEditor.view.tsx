...
import { ItemEditorFormFields, type ItemFormData } from "./ItemEditorFormFields";
...
export function ItemEditorView({
  item, formData, loading, usdaQuery, usdaLoading, hasSearchHandler,
  onClose, onQueryChange, onSearch, handleSubmit, handleChange,
  handleMacroChange, handleAllergensChange
}: ItemEditorViewProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/50 dark:bg-black/60 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center sticky top-0 bg-white dark:bg-slate-900 z-10">
          ...
          <button onClick={onClose} className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
            <X size={20} />
          </button>
        </div>
