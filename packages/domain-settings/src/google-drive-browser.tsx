...
import { Loader2, Search, X, Folder, FileText, CheckCircle } from "lucide-react";
...
/** Container: Google Drive browser modal for selecting and importing Drive files. */
export function GoogleDriveBrowser({ isOpen, onClose, documentType = "RECIPE", onSearch, onImport }: GoogleDriveBrowserProps) {
  ...
  const handleSearch = useCallback(async (q: string, folderId?: string) => {
    setLoading(true);
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/50 dark:bg-black/60 backdrop-blur-sm">
      <div className="bg-zinc-100 dark:bg-card border border-black/10 dark:border-white/10 rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[85vh] animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-4 border-b border-black/5 dark:border-white/5">
          ...
          <button onClick={onClose} className="text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {loading ? (
            ...
            <div className="text-center py-12 text-zinc-400 text-sm">No files found.</div>
          ) : (
              ...
              {files.map((f) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isFolder && (
                      <button
                        onClick={(e) => { e.stopPropagation(); setCurrentFolder({ id: f.id, name: f.name }); setQuery(""); handleSearch("", f.id); }}
        ...
        <div className="p-4 border-t border-black/5 dark:border-white/5 flex justify-between items-center bg-card/80 rounded-b-2xl">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleImport} disabled={selectedIds.size === 0 || loading} className="px-4 py-2 rounded-lg text-sm font-semibold bg-sky-500 hover:bg-sky-400 text-white disabled:opacity-50 transition-colors flex items-center gap-2 cursor-pointer">
