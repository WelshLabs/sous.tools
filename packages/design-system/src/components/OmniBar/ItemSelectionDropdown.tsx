...
interface ItemSelectionDropdownProps {
  ...
  suggestions: Array<{ itemId: string; name: string; similarity: number; matchColor: string }> | undefined;
      ...
      <div className="overflow-y-auto flex-1 py-1 text-xs">
        {suggestions && suggestions.length > 0 && !search && (
          <div className="border-b border-slate-100 dark:border-zinc-900 pb-1 mb-1 animate-fadeIn">
            {suggestions.map((sug) => (
              <div key={sug.itemId} onClick={() => onSelectItem(sug.itemId, sug.similarity)} className="px-3 py-1.5 cursor-pointer hover:bg-sky-500 hover:text-white dark:hover:bg-sky-600 flex items-center justify-between">
        ...
        {filteredItems.length > 0 ? (
          filteredItems.map((opt) => (
            <div key={opt.id} onClick={() => onSelectItem(opt.id, 1.0)} className={`px-3 py-1.5 cursor-pointer hover:bg-sky-500 hover:text-white dark:hover:bg-sky-600 flex items-center justify-between ${opt.id === currentItemId ? "bg-slate-100 dark:bg-zinc-800 text-emerald-500 font-semibold" : ""}`}>
        ...
        {search.trim() !== "" && !filteredItems.some((o) => o.name.toLowerCase() === search.trim().toLowerCase()) && (
          <div
