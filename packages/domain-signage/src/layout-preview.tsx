...
export const LayoutPreview: React.FC<LayoutPreviewProps> = ({
  ...
  useEffect(() => {
    if (config.aspectRatio === "responsive" || config.scaleToFit === false)
      return;
  ...
  if (googleFontsUrl)
    combinedCustomCss += `@import url('${googleFontsUrl}');\n`;
  ...
  const previewContent = (
    <div
      className={`w-full flex-1 min-h-[100vh] relative st-layout-background flex flex-col ${config.aspectRatio === "responsive" ? "" : "border-2 border-border shadow-2xl rounded-2xl"}`}
      style={bgStyle}
    >
      {!activeSlide ? (
  ...
  return (
    <div
      className="w-full min-h-full relative flex items-start justify-center signage-preview-container bg-background dark:bg-background pt-8 pb-32 st-signage-root"
      ref={containerRef}
    >
