...
export function PreviewNestedItem({
    ...
    >
      <div className="flex justify-between font-bold">
        <span
          ...
          className={
            isGroupHeader
              ? "text-[#00f0ff] font-brand st-menu-glow-text text-[10px] tracking-widest uppercase st-category-header"
              : "st-menu-item-title"
          }
        >
          {baseName}
      ...
      {baseDesc && (
        <div
          ...
          className="text-[8px] opacity-80 -mt-0.5 mb-1 leading-snug"
        >
          {baseDesc}
        </div>
      )}
      <ul
        className={`flex flex-col gap-0.5 text-[8px] opacity-80 ${isGroupHeader ? "" : "pl-2 border-l border-border"}`}
      >
            ...
            <li key={idx} className="flex flex-col mb-1">
              ...
              {upDesc && (
                <div
                  ...
                  className="text-[7px] opacity-70 pl-2 leading-tight"
                >
                  {upDesc}
                </div>
              )}
  ...
  return isGlass ? (
    <div
      className={["st-glass-panel p-2 rounded st-nested-item", block.className]
        .filter(Boolean)
        .join(" ")}
