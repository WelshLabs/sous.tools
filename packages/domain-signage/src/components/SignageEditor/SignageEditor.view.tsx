...
export const SignageEditorView: React.FC<SignageEditorViewProps> = ({
  config, items, activeSlideIndex, isPlaying, setIsPlaying, isWorkspaceOpen, setIsWorkspaceOpen,
  viewMode, setViewMode, selectedBlockId, setSelectedBlockId, showOutlines, setShowOutlines,
  containerRef, isDraft, discardDraft, clearDraftOnSave, updateConfig, updateSlide, selectSlide,
  handleAddSlide, handleUpdateBlock, handleDragEnd, activeBlock, saving, onSave, layoutName,
  deckId, deckSlug, onRenameDeck, onFetchModifierGroups, onFetchModifierOptions,
}) => {
  return (
    <div className="flex flex-col h-full bg-background text-foreground">
      ...
      {viewMode === "preview" && (
        <div ref={containerRef} className="fixed inset-0 z-50 bg-background dark:bg-background flex items-center justify-center overflow-hidden">
          {showOutlines && (
            <style>{`
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-4 px-6 py-3 rounded-full bg-background/80 backdrop-blur-xl border border-border shadow-2xl z-[60]">
             <button onClick={() => setShowOutlines(!showOutlines)} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors ${showOutlines ? "bg-cyan-500 text-foreground" : "bg-muted/50 text-muted-foreground hover:bg-background/10 dark:bg-background/10"}`}>
