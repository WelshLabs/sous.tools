...
export const SignagePreviewView: React.FC<SignagePreviewViewProps> = (
  ...
  return (
    <div
      className="w-full min-h-full relative flex flex-col items-center justify-start signage-preview-container bg-background dark:bg-background pt-8 pb-32 st-signage-root"
      ref={containerRef}
    >
      ...
      config.scaleToFit !== false ? (
        <div
          className="w-[1920px] h-[1080px] shrink-0 origin-top transform-gpu shadow-2xl"
...
const PreviewContent = (props: any) => {
  ...
  return (
    <div
      className={`w-full flex-1 h-full relative st-layout-background flex flex-col ${config.aspectRatio === "responsive" ? "" : "border-2 border-border shadow-2xl rounded-2xl"}`}
      style={bgStyle}
    >
      {!activeSlide ? (
      ...
      {(config.overlays ?? []).map((o: any) => (
        <div
          key={o.id}
          className={`absolute text-[9px] bg-background/80 border border-border px-1.5 py-0.5 rounded shadow signage-overlay ${o.customCssClass ?? ""}`}
          style={{
            top: o.position.top,
        ...
        >
          {o.type === "BADGE" && (
            <span className="bg-red-500 text-foreground font-bold px-0.5 rounded mr-0.5 text-[8px]">
              SOLD OUT
            </span>
          )}
          {o.content}
...
const PreviewContentBlocks = ({
  ...
  switch (block.type) {
    ...
    case "ImageBlock":
      return (
        <div className="w-full h-full min-h-[60px] flex items-center justify-center bg-background/50 border border-dashed border-black/10 rounded overflow-hidden">
          {block.imageUrl ? (
    ...
    case "VideoBlock":
      return (
        <div className="w-full h-full min-h-[200px] flex items-center justify-center bg-background/50 border border-dashed rounded">
          <video
            src={block.videoUrl || ""}
            autoPlay
            loop
    ...
    default:
      return (
        <div className="w-full min-h-[100px] flex items-center justify-center bg-muted/50 border border-dashed rounded">
...
const PreviewCategoryHeader = ({ block }: any) => {
  ...
  return (
    <div
      className={`w-full p-2 rounded flex flex-col gap-0.5 ${block.panelStyle === "glass" ? "st-glass-panel" : ""} ${block.className || ""}`}
    >
...
const PreviewPosItem = ({ block, items, styles }: any) => {
  ...
  return (
    <div
      className={`p-1.5 rounded flex justify-between items-center text-[9px] ${block.className || ""}`}
      style={{
...
const PreviewMenuList = ({ block, items, styles }: any) => {
  if (!block.itemIds?.length)
    return (
      <div className="text-[10px] text-muted-foreground">Select items...</div>
    );
  return (
    ...
    <div className="relative overflow-hidden w-full h-full min-h-[200px]">
      {block.slides.map((s: any, i: number) => (
        <img
          key={i}
          src={s.imageUrl}
          className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-700 ${i === idx ? "opacity-100 z-10" : "opacity-0 z-0"}`}
        />
      ))}
    </div>
  );
...
const PreviewModifierGroup = ({ block, fetchModifiers }: any) => {
  ...
  return (
    <div className="w-full flex flex-col divide-y divide-white/5">
      <div className="px-3 py-2 font-semibold text-[10px] uppercase">
        Options
      </div>
      {opts.map((o) => (
      ...
      <Droppable droppableId="filmstrip-slides" direction="horizontal">
        {(provided) => (
          ...
          >
                ...
                {(drag) => (
                  ...
                  >
                    ...
                    <div
                      ...
                      className={`w-32 h-20 border rounded cursor-pointer ${index === activeSlideIndex ? "border-cyan-500" : "border-white/20"}`}
                    >
                      Slide {index + 1}
                    </div>
                  </div>
            ...
            <button
              onClick={onAddSlide}
              className="w-32 h-20 border-2 border-dashed flex items-center justify-center text-xs"
            >
