  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-2 py-2.5 border-b border-border shrink-0">
        {!isCollapsed && (
          <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest ml-1">Reference</span>
        )}
        <button
              ...
              {Object.entries(CSS_DICTIONARY).map(([category, classes]) => (
                ...
                >
                  ...
                  <div className="px-2 pb-2 space-y-1.5 pt-1 border-t border-border bg-background/30">
                    {classes.map((d) => (
                      <div key={d.className} className="flex justify-between items-start gap-1 p-1.5 rounded hover:bg-secondary text-[11px]">
                        <div className="min-w-0">
                          <span className="font-mono text-blue-400 font-semibold truncate block">{d.className}</span>
                          <span className="text-muted-foreground text-[10px] block">{d.desc}</span>
                        </div>
            ...
            <div className="space-y-2">
              {CSS_PRESETS.map((p) => (
                ...
                >
                  ...
                  <div className="text-[9px] text-muted-foreground truncate font-mono mt-0.5">{p.css.split("\n")[0]}...</div>
                </button>
              ))}
            </div>
          </div>
