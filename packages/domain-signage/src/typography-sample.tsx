...
export const TypographySample: React.FC<{
  ...
  <div className="relative flex flex-col gap-1">
    <div className="flex justify-between items-center">
      <p className="text-[10px] text-zinc-400 dark:text-zinc-500">{sample.label}</p>
      <input
        type="color"
        value={color || "#ffffff"}
        onChange={(e) => onColorChange(e.target.value)}
        className="w-5 h-5 rounded cursor-pointer border border-black/10 dark:border-white/10 bg-zinc-800 p-0"
      />
    </div>
    <button
      onClick={onToggle}
      className="w-full text-left px-2 py-1.5 bg-zinc-800 border border-black/10 dark:border-white/10 rounded-lg hover:border-white/20 transition-all cursor-pointer"
      style={{ fontFamily: font ?? "inherit", color: color || "inherit" }}
    >
      <span className="text-[10px] font-bold truncate block">{sample.sample}</span>
      <span className="text-[8px] text-zinc-400 dark:text-zinc-500 block">{font ?? "inherit"}</span>
    </button>
    {isOpen && (
