        ...
        return (
          ...
          >
            <input
              ...
              onChange={(e) => {
                if (e.target.checked)
                  onUpdateBlock(selectedBlockId, { modifierGroupId: g.id });
              }}
...
export const RightSidePanel: React.FC<RightSidePanelProps> = ({
  ...
  const [activeTab, setActiveTab] = React.useState<
    "settings" | "blocks" | "layers" | "block-settings"
  >("settings");
  ...
  React.useEffect(() => {
    if (selectedBlockId) {
      if (
        selectedBlock &&
        ["ColumnBlock", "RowBlock", "GridBlock"].includes(selectedBlock.type)
      ) {
        const hasChildren =
          (selectedBlock as any).blocks?.length > 0 ||
          (selectedBlock as any).cells?.length > 0;
        if (!hasChildren) {
          setActiveTab("blocks");
        } else {
  ...
  function findParentExplodedItem(
    root: SignageBlock,
    childId: string,
  ): SignageBlock | null {
    ...
    if (
      root.type === "ColumnBlock" ||
      root.type === "RowBlock" ||
      root.type === "ExplodedItemBlock"
    ) {
      if (root.blocks) {
        for (const b of root.blocks) {
  ...
  return (
    <div
      className={`absolute right-0 top-0 bottom-0 z-30 w-96 flex flex-col bg-zinc-50 dark:bg-zinc-950 border-l border-black/5 dark:border-white/5 shadow-2xl h-full overflow-hidden transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"}`}
    >
                        ...
                        for (const col of activeSlide.columns) {
                          if (col.blocks) {
                            for (const root of col.blocks) {
                              const found = findParentExplodedItem(
                                root,
                                selectedBlockId,
                              );
