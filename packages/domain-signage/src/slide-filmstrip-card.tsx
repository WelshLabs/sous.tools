...
export function SlideCard({
  ...
  return (
    <div
      onClick={onSelect}
      className={[
        "group relative h-[90px] w-40 flex-shrink-0 cursor-pointer overflow-hidden rounded-lg transition-shadow duration-150",
        isActive ? "ring-primary ring-2" : "ring-1 ring-white/10",
      ].join(" ")}
    >
      <SlideMiniature slide={slide} />
      <button
        onClick={onRemove}
        className="bg-background/50 dark:bg-background/60 absolute top-1 right-1 z-10 rounded p-0.5 opacity-0 transition-opacity group-hover:opacity-100 hover:bg-red-500/80"
        aria-label="Remove slide"
      >
        <Trash2 className="text-foreground h-3 w-3" />
      </button>
    </div>
  );
}
