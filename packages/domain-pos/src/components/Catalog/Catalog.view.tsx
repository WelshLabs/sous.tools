...
export interface PosModifierGroup {
  ...
  name: string;
  external_id: string;
  pos_modifier_options?: Array<{ id: string; name: string; price: number }>;
}
...
export type CatalogTabType = "ITEMS" | "CATEGORIES" | "MODIFIERS" | "DISCOUNTS";

export interface CatalogViewProps {
  activeTab: CatalogTabType;
  setActiveTab: (tab: CatalogTabType) => void;
  ...
  editingItem: PosCatalogItem | null;
  editName: string;
  ...
  editSoldOut: boolean;
  setEditSoldOut: (val: boolean) => void;
  saving: boolean;
  onStartEdit: (item: PosCatalogItem) => void;
  onCancelEdit: () => void;
  ...
  return (
    <div className="animate-fadeIn relative mx-auto max-w-7xl space-y-6 pb-12">
      ...
      <div className="flex space-x-2 border-b border-black/5 pb-2 dark:border-white/5">
        {(
          ["ITEMS", "CATEGORIES", "MODIFIERS", "DISCOUNTS"] as CatalogTabType[]
        ).map((tab) => (
          <button
            key={tab}
          ...
          {modifierGroups.map((mg) => (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                {mg.pos_modifier_options?.map((opt) => (
                  <div
                    key={opt.id}
