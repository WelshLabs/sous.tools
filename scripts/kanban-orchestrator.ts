...
function runCommand(command: string, hideOutput: boolean = false): { stdout: string; stderr: string; exitCode: number } {
  try {
    if (!hideOutput) console.log(`\n[RUNNING] ${command}`);
    const stdioOpt = hideOutput ? "pipe" : "inherit";
    const stdout = execSync(command, { encoding: "utf-8", stdio: ["inherit", stdioOpt, stdioOpt] });
...
async function main() {
  ...
  if (commentBody) {
    if (commentBody.includes("/approve")) {
      ...
      runCommand(`gh issue edit ${issueNumber} --repo ${repo} --add-label "Done"`);
      return;
    }
  ...
  const commentsRes = runCommand(`gh issue view ${issueNumber} --repo ${repo} --comments ${COMMENT_FETCH_LIMIT} --json comments -q ".comments[].body"`, true);
  ...
  while (attempt < maxAttempts) {
    ...
    // Run Aider
    const aiderCmd = `aider --model ${currentModel} --openai-api-base ${LITELLM_URL} --openai-api-key sk-1234 --edit-format diff --no-auto-commits --yes ${fileArg} --message-file ${promptFile}`;
    ...
    console.log("[ORCHESTRATOR] Running deterministic auto-fixes (eslint/prettier)...");
  ...
  const commitMsg = (event === "ready" || event === "Ready") ? `feat(issue-${issueNumber}): ${issueTitle}` : `fix(issue-${issueNumber}): self-repair or human feedback`;
  runCommand(`git add .`);
  ...
  const prExists = runCommand(`gh pr view issue-${issueNumber} --repo ${repo}`, true);
  if (prExists.exitCode !== 0) {
    runCommand(`gh pr create --repo ${repo} --base main --head issue-${issueNumber} --title "${commitMsg}" --body "${prBody}"`);
  } else {
