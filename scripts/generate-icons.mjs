...
const devGreen = "#22c55e";
const stagOrange = "#f97316";
const prodGradient = "url(#bg)";
...
function svg({
  ...
  const pad = maskable ? 0 : micro ? 5 : 12;
...
await Promise.all([
  ...
  png("pwa-192.png", 192),
  png("pwa-512.png", 512),
  png("maskable-192.png", 192, maskableSvg),
  png("maskable-512.png", 512, maskableSvg),
  png("macos-1024.png", 1024),
  ...[16, 32, 48, 64, 128, 256, 512].map((size) =>
    png(`linux-${size}.png`, size, size <= 32 ? microSvg : standardSvg),
  ),
]);
const windowsSizes = await Promise.all(
  [16, 24, 32, 48, 64, 128, 256].map((size) =>
    png(`windows-${size}.png`, size, size <= 32 ? microSvg : standardSvg),
  ),
);
await writeFile(
  path.join(out, "favicon.ico"),
  await pngToIco([browser16, browser32, windowsSizes[3]]),
);
