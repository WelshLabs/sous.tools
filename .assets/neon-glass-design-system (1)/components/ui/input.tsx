...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, label, hint, error, icon, trailing, id, disabled, value, defaultValue, onChange, onFocus, onBlur, placeholder, ...props }, ref) => {
        ...
        >
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={label ? (floating && focused ? placeholder : undefined) : placeholder}
          ...
          {trailing && <span className="ml-2 inline-flex shrink-0 items-center">{trailing}</span>}
        ...
        <AnimatePresence initial={false} mode="wait">
          {(error || hint) && (
            <motion.p
              ...
              className={cn("px-1 text-xs", error ? "text-destructive" : "text-muted-foreground")}
            >
              {error ?? hint}
            </motion.p>
          )}
