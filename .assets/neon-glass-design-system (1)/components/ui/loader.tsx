...
import { AnimatedLettermark } from "@/components/logo"
...
export function BrandLoader({ size, className, label = "Loading", gradient = true }: BaseProps & { gradient?: boolean }) {
  return (
    <div role="status" aria-label={label} className={cn("inline-flex items-center justify-center", className)}>
      <AnimatedLettermark gradient={gradient} className={cn(loaderSize({ size }))} />
  ...
  return (
    <div role="status" aria-label={label} className={cn("inline-flex", className)}>
      <span
        ...
        style={{
          background: "conic-gradient(from 0deg, transparent 0%, var(--primary) 70%, var(--accent) 100%)",
...
export function DotsLoader({ className, label = "Loading" }: { className?: string; label?: string }) {
  return (
    <div role="status" aria-label={label} className={cn("inline-flex items-center gap-1.5", className)}>
      {[0, 1, 2].map((i) => (
        <motion.span
          key={i}
          className="h-2.5 w-2.5 rounded-full bg-primary shadow-glow-sm"
          animate={{ scale: [1, 1.5, 1], opacity: [0.4, 1, 0.4] }}
          transition={{ duration: 1, repeat: Number.POSITIVE_INFINITY, delay: i * 0.15, ease: "easeInOut" }}
        />
      ))}
      <span className="sr-only">{label}</span>
    </div>
