 ...
 * A slim neon gradient bar pinned to the top edge that trickles toward ~90%
 * while `active`, then snaps to 100% and fades out when `active` flips false.
 *
 * Usage:
 *   const [loading, setLoading] = useState(false)
 *   <TopProgress active={loading} />
 *
 * Pass `absolute` to pin it to the nearest positioned parent instead of the
 * viewport (handy for demos inside a card).
 */
  ...
  React.useEffect(() => {
    ...
    if (active) {
      ...
      trickle.current = setInterval(() => {
        ...
        setProgress((p) => (p >= 90 ? p : Math.min(90, p + (100 - p) * 0.08 + Math.random() * 1.5)))
      }, 380)
    } else if (visible) {
      clearTrickle()
      setProgress(100)
      ...
      {visible && (
        ...
        >
          <motion.div
            ...
            style={{ boxShadow: "var(--ds-glow-md)" }}
            initial={false}
