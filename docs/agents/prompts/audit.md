...
Choose exactly ONE file with an error and refactor it into compliance. Commit the fix to a new branch named "audit/fix-{ISSUE_NUMBER}" and push the branch. Then open a Pull Request in the "conarwelsh/sous.tools" repository using the Github MCP Server. DO NOT output conversational text in the terminal. DO NOT converse. Execute the tools and stop.