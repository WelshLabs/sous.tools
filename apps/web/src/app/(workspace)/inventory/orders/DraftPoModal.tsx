    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/80 dark:bg-black/80 backdrop-blur-sm">
      <div className="glass-panel p-8 max-w-2xl w-full">
        ...
        <div className="mb-6 space-y-2">
          <label className="text-sm font-medium text-muted-foreground">Select Vendor</label>
          <select value={selectedVendor} onChange={e => setSelectedVendor(e.target.value)} className="w-full bg-white/50 dark:bg-black/60 border border-white/20 rounded-md p-3 text-foreground">
        ...
        <div className="flex justify-end gap-4">
          <button onClick={onClose} disabled={isSubmitting} className="px-6 py-2 rounded-md font-medium hover:bg-card dark:bg-white/10 transition-colors disabled:opacity-50">
            Cancel
          </button>
          <button 
            onClick={createPO}
            disabled={!selectedVendor || selectedItems.size === 0 || isSubmitting} 
            className="bg-white text-foreground px-6 py-2 rounded-md font-medium hover:bg-gray-200 disabled:opacity-50 transition-colors"
          >
