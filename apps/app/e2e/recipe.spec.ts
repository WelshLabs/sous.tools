...
test.describe('Recipe Engine E2E', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/auth/v1/token?grant_type=password', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          user: { id: 'd0000000-0000-0000-0000-000000000000', email: 'conar@dtown.cafe', role: 'authenticated' }
        })
      });
    });
    ...
    await page.route('**/auth/v1/user', async route => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ id: 'd0000000-0000-0000-0000-000000000000', email: 'conar@dtown.cafe', role: 'authenticated' })
      });
    });
    ...
    await page.route('**/api/recipes/vessels', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          success: true,
          data: [
            { id: 'v-1', name: '9" Pullman Pan', shape: 'RECTANGULAR', length: 23, width: 10, height: 10, volumeMl: 2300, createdAt: '' }
          ]
        })
      });
    });
    ...
    await page.route('**/api/recipes/ingredients', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          success: true,
          data: [
            { id: 'i-1', name: 'Bread Flour', densityGMl: 0.57, nutritionMacros: { calories: 364 }, allergens: ['wheat'] }
          ]
        })
      });
    });
            ...
            data: [
              {
                ...
                yieldCount: 2,
                yieldUnit: 'loaves',
                instructions: [{ stepNumber: 1, text: 'Mix flour and water.', timerDurationSeconds: 300 }]
              }
            ]
          })
        });
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            vesselId: 'v-1',
            instructions: [{ stepNumber: 1, text: 'Mix flour and water.', timerDurationSeconds: 300 }],
            vessel: { id: 'v-1', name: '9" Pullman Pan', shape: 'RECTANGULAR', volumeMl: 2300 },
            recipeIngredients: [
              { id: 'ri-1', recipeId: 'rec-1', masterIngredientId: 'i-1', calculationType: 'fixed_weight', baseCalculationGroup: true, amount: 500, unit: 'g', prepNotes: '', masterIngredient: { id: 'i-1', name: 'Bread Flour', densityGMl: 0.57 } }
            ]
          }
        })
      });
  ...
  test('should navigate to recipes, view, scale, and start active kitchen mode', async ({ page }) => {
    // Login
    ...
    await expect(page.locator('h3:has-text("Hybrid Scaling Tool")')).toBeVisible();
  ...
  test('should navigate to vessels manager and see list of vessels', async ({ page }) => {
    await page.goto('/login');
    await page.fill('input[type="email"]', 'conar@dtown.cafe');
    await page.fill('input[type="password"]', 'password');
    await page.click('button:has-text("Sign In")');
