...
export function RecipeCostPanel({ recipeId }: { recipeId: string }) {
  ...
  if (loading) {
    return (
      <div className="bg-zinc-900 border border-white/5 rounded-2xl p-4 shadow-xl glass-panel animate-pulse space-y-3">
  ...
  if (!costData || costData.ingredients.length === 0) {
    return (
      <div className="bg-zinc-900 border border-white/5 rounded-2xl p-4 shadow-xl glass-panel text-amber-400 text-sm text-center">
  ...
  return (
    <div className="bg-zinc-900 border border-white/5 rounded-2xl p-4 shadow-xl glass-panel space-y-4 text-white">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={saveVersion}
          disabled={saving}
          className="flex items-center gap-1 text-xs bg-zinc-800 hover:bg-zinc-750 border border-white/10 px-2 py-1 rounded transition"
        >
          {savedText ? "✓ Saved" : (
            <>
        ...
        <table className="w-full text-xs text-left text-zinc-300">
          ...
          <tbody>
            {costData.ingredients.map((ing) => (
              <tr key={ing.ingredientId} className="border-b border-white/5">
