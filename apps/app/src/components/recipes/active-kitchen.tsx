...
export const ActiveKitchen: React.FC<ActiveKitchenProps> = ({ recipeId }) => {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black text-zinc-900 dark:text-slate-100 p-6 flex flex-col justify-between w-full">
      <header className="flex justify-between items-center pb-4 border-b border-zinc-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black font-brand text-zinc-900 dark:text-slate-100 uppercase tracking-wide">{recipe.title}</h2>
        ...
        <div className="flex items-center gap-2">
          {wakeLockActive ? (
            ...
            <span className="flex items-center gap-1 text-[10px] bg-zinc-800 text-zinc-400 dark:text-zinc-500 px-2.5 py-1 rounded-full font-bold border border-zinc-700/40 uppercase tracking-wider"><Sun className="w-3.5 h-3.5 opacity-40" /> Wake Lock Offline</span>
          )}
        </div>
      ...
      <main className="flex-1 py-8 space-y-6">
        {recipe.instructions.map((step) => {
          ...
          return (
            <div key={step.stepNumber} className={`p-6 rounded-2xl border transition-all flex items-start gap-4 cursor-pointer select-none ${isChecked ? "bg-zinc-950/40 border-zinc-900 opacity-40" : "bg-zinc-100 dark:bg-zinc-900 border-black/5 dark:border-white/5 hover:border-slate-800"}`} onClick={() => toggleStepCheck(step.stepNumber)}>
