...
export function VersionHistoryDrawer({ recipeId, isOpen, onClose }: VersionHistoryDrawerProps) {
  ...
  return (
    <div
      className={`fixed inset-y-0 right-0 w-80 bg-zinc-900 border-l border-white/5 shadow-2xl glass-panel z-50 p-4 transition-transform duration-300 transform ${
    ...
    >
      ...
      <div className="mt-4 space-y-3 overflow-y-auto h-[calc(100vh-80px)]">
        {loading ? (
          <div className="text-center text-xs text-zinc-500 py-8 animate-pulse">Loading history...</div>
        ) : versions.length === 0 ? (
          ...
          versions.map((ver) => (
            <div
              key={ver.id}
              className="bg-zinc-850 border border-white/5 rounded-lg p-3 text-zinc-300 space-y-2"
            >
