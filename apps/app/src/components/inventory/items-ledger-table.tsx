...
export default function ItemsLedgerTable({ items, loading, onEdit, onDelete }: ItemsLedgerTableProps) {
  if (loading) {
    return <div className="p-12 text-center text-zinc-400 dark:text-zinc-500">Loading ledger...</div>;
  }

  if (!items.length) {
    return <div className="p-12 text-center text-zinc-400 dark:text-zinc-500">No items found in ledger.</div>;
  }

  return (
    <div className="w-full overflow-x-auto">
      <table className="w-full text-left text-sm text-zinc-700 dark:text-zinc-300">
        <thead className="bg-zinc-900/80 text-zinc-500 dark:text-zinc-400 uppercase font-semibold text-xs border-b border-black/5 dark:border-white/5">
          <tr>
            ...
            <tr key={item.id} className="hover:bg-black/5 dark:bg-white/5 transition-colors group">
              ...
              <td className="px-6 py-4 text-right">
                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => onEdit(item)} className="p-2 text-zinc-500 dark:text-zinc-400 hover:text-sky-400 bg-black/5 dark:bg-white/5 hover:bg-sky-500/10 rounded-lg transition-colors">
                    <Edit size={16} />
                  </button>
                  <button onClick={() => onDelete(item.id)} className="p-2 text-zinc-500 dark:text-zinc-400 hover:text-red-400 bg-black/5 dark:bg-white/5 hover:bg-red-500/10 rounded-lg transition-colors">
                    <Trash2 size={16} />
                  </button>
                </div>
              </td>
