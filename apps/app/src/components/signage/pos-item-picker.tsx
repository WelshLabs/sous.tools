    ...
    <div className="flex flex-col gap-2 w-full" ref={containerRef}>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-1.5 bg-zinc-900 border border-white/10 rounded-lg text-xs focus:outline-none focus:border-cyan-500 text-zinc-100 placeholder:text-zinc-600 transition-colors"
        />
        
        {/* Dropdown Options */}
        {isOpen && (
          <div className="absolute z-50 top-full mt-1.5 w-full bg-zinc-900 border border-white/10 rounded-lg shadow-2xl max-h-48 overflow-y-auto custom-scrollbar">
            {filtered.length === 0 ? (
              <div className="text-xs text-zinc-500 italic p-3 text-center bg-zinc-950/50">No available items found.</div>
            ) : (
              <div className="flex flex-col p-1">
                {filtered.map(item => (
                  <button
                    ...
                    className="flex items-center justify-between w-full text-left px-2 py-2 rounded text-xs text-zinc-300 hover:bg-white/5 hover:text-zinc-100 transition-colors"
                  >
                    <span className="truncate pr-4">{item.name}</span>
                    <span className="shrink-0 font-mono text-[10px] text-zinc-500">${Number(item.price).toFixed(2)}</span>
                  </button>
                ))}
              </div>
            )}
