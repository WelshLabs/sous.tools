...
export function BlockEditorNode({
  ...
  const baseClasses = `relative transition-all border-2 rounded-lg ${
    isSelected ? "border-[oklch(0.7_0.15_200)] shadow-[0_0_15px_oklch(0.7_0.15_200/0.5)] z-20" : "border-dashed border-white/10 hover:border-white/30 z-10"
  ...
  const renderEmptyState = (layoutType?: "row" | "column" | "grid") => {
    ...
    return (
      <div
        ...
        className="flex flex-col items-center justify-center w-full h-full min-h-[120px] border-2 border-dashed border-white/20 hover:border-cyan-400 bg-white/5 hover:bg-cyan-900/20 rounded-xl cursor-pointer transition-all group p-4"
      >
          ...
          <Droppable droppableId={block.id!} type="BLOCK" direction="vertical">
            {(dropProps) => (
              <div ref={dropProps.innerRef} {...dropProps.droppableProps} className={`${getLayoutClass("column", block.panelStyle, block.className)} pt-8`} onClick={handleSelect}>
                <div className="absolute top-0 left-0 px-2 py-1 bg-zinc-800 border-b border-r border-zinc-700 rounded-br-lg rounded-tl-lg text-[9px] font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-1 z-30 pointer-events-none">
                  {dragProps && (
                    <div {...dragProps.dragHandleProps} className="cursor-grab active:cursor-grabbing pointer-events-auto flex items-center justify-center p-0.5 hover:bg-white/10 rounded">
    ...
    case "ExplodedItemBlock":
      ...
      return renderDraggable((dragProps) => (
        <div ref={dragProps?.innerRef} {...dragProps?.draggableProps} style={{ ...sizingStyles, ...dragProps?.draggableProps.style }} className={`${baseClasses} flex flex-col overflow-hidden`}>
          <div className="absolute top-0 left-0 px-2 py-1 bg-zinc-800 border-b border-r border-zinc-700 rounded-br-lg rounded-tl-lg text-[9px] font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-1 z-30 pointer-events-auto cursor-pointer hover:bg-zinc-700 transition-colors" onClick={(e) => { e.stopPropagation(); onSelectBlock(block.id!); }}>
            {dragProps && (
              <div {...dragProps.dragHandleProps} className="cursor-grab active:cursor-grabbing flex items-center justify-center p-0.5 hover:bg-white/10 rounded" onClick={(e) => e.stopPropagation()}>
                <GripVertical className="w-3 h-3 text-cyan-400" />
              </div>
            )}
          ...
          <div className="pt-8 px-4 pb-2 border-b border-white/5 flex flex-col gap-1">
            {explodedItem ? (
              <>
                ...
                {!(block as any).hideDescription && explodedItem.description && <span className="text-[10px] text-zinc-400">{explodedItem.description}</span>}
              </>
            ) : (
