...
const TreeNode = ({ block, depth, selectedBlockId, onSelectBlock }: { block: SignageBlock, depth: number, selectedBlockId?: string | null, onSelectBlock: (id: string | null) => void }) => {
  ...
  return (
    <div className="flex flex-col">
      <div 
        className={`flex items-center gap-1.5 py-1.5 px-2 rounded-md cursor-pointer transition-colors ${isSelected ? "bg-cyan-500/20 text-cyan-400" : "hover:bg-white/5 text-zinc-300"}`}
        style={{ paddingLeft: `${depth * 12 + 8}px` }}
        onClick={() => onSelectBlock(block.id || null)}
      >
        {isContainer ? (
           <button onClick={(e) => { e.stopPropagation(); setExpanded(!expanded); }} className="p-0.5 hover:bg-white/10 rounded">
  ...
  return (
    <div className="flex flex-col flex-1 overflow-y-auto p-2">
      <div className="flex flex-col gap-1">
        <div className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 px-2 py-1 mb-1">Root Layout</div>
