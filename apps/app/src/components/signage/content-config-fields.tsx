...
export function ContentConfigFields({
  ...
  const renderPosItemBinding = () => {
    ...
    return (
      <div className="space-y-3">
        <label className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 block">POS Item Selection</label>
  ...
  const renderKenBurnsParams = () => {
    return (
      <div className="space-y-3">
        <label className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 block">Ken Burns Settings</label>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase font-bold block mb-1">Zoom Speed (s)</label>
            <input
              type="number"
              min={1}
              max={60}
              placeholder="10"
              className="w-full bg-zinc-100 dark:bg-zinc-900 border border-black/10 dark:border-white/10 rounded-lg px-2 py-1.5 text-xs text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-cyan-500"
              onChange={(e) => onChange({ carouselSettings: { ...((block as any).carouselSettings || {}), speed: Number(e.target.value) } } as any)}
            />
          </div>
          <div>
            <label className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase font-bold block mb-1">Max Zoom Scale</label>
            <input
              type="number"
              step="0.05"
              min="1.0"
  ...
  const renderOosModifiers = () => {
    ...
    return (
      <div className="space-y-3">
        <label className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 block">Out-of-Stock Modifier Behavior</label>
            ...
            <button
              ...
              className={`py-2 px-1 text-center rounded-lg text-xs font-bold transition border ${
                oosBehavior === mode
                  ...
                  : "bg-zinc-100 dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 border-black/10 dark:border-white/10 hover:border-white/20"
              }`}
            >
