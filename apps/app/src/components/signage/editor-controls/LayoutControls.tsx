...
export function LayoutControls({ block, onUpdate }: LayoutControlsProps) {
  ...
  const parseSpacing = (spacingStr: string | undefined): [string, string, string, string] => {
  ...
  const handleSpacingChange = (
    ...
    const val = newArr.every(v => v === newArr[0]) ? newArr[0] : newArr.join(" ");
        ...
        <div className="grid grid-cols-4 gap-1">
          {["T", "R", "B", "L"].map((dir, i) => (
            <div key={dir} className="relative">
              <span className="absolute left-1.5 top-1/2 -translate-y-1/2 text-[9px] text-zinc-600 font-bold">{dir}</span>
              <input 
                type="text" 
                value={values[i]} 
  ...
  return (
    <div className="space-y-4">
      ...
      <SpacingControl 
        ...
        onChange={(idx, val, linked) => handleSpacingChange("padding", idx, val, linked, pad)} 
      />
      ...
      <SpacingControl 
        ...
        onChange={(idx, val, linked) => handleSpacingChange("margin", idx, val, linked, mar)} 
      />
    </div>
  );
}
