  ...
  return (
    <div className="relative w-full">
      <button
        ...
        className="flex items-center justify-between w-full bg-slate-900 border border-slate-700 hover:border-slate-500 rounded px-2.5 py-1.5 text-xs text-slate-200 cursor-pointer"
      >
        <span>
        ...
        <div className="absolute left-0 right-0 mt-1 z-30 bg-[oklch(0.16_0.02_180)] border border-[oklch(0.26_0.03_180)] rounded-lg shadow-xl p-2 max-h-60 overflow-y-auto space-y-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-700 rounded pl-8 pr-2 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-primary placeholder-slate-500"
            />
          </div>
          ...
          <div className="space-y-1">
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <div className={`w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 ${
                      isSelected ? "border-primary bg-primary text-primary-foreground" : "border-slate-600"
                    }`}>
                      {isSelected && <Check className="w-2.5 h-2.5 stroke-[3]" />}
                    </div>
                    <div className="truncate">
                      <div className="text-xs font-semibold truncate">{item.name}</div>
                      <div className="text-[10px] text-slate-400 font-mono">${Number(item.price).toFixed(2)}</div>
                    </div>
                  </div>
                  ...
                  {isSelected && (
                    <button
                      ...
                      className={`p-1 rounded hover:bg-slate-700/50 transition-colors shrink-0 ${
                        isHighlighted ? "text-amber-400" : "text-slate-500 hover:text-slate-300"
                      }`}
                    >
            ...
            {filteredItems.length === 0 && (
              <div className="text-center text-[10px] text-slate-500 p-2 font-mono">No items found</div>
            )}
          </div>
        </div>
      )}
