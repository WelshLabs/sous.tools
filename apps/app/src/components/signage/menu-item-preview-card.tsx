...
const Tooltip: React.FC<{ label: string }> = ({ label }) => (
  <span className="pointer-events-none absolute -top-6 left-1/2 -translate-x-1/2 z-50 whitespace-nowrap px-2 py-0.5 bg-zinc-800 border border-white/10 rounded-full text-[9px] text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity">
    {label}
  </span>
);
...
export const MenuItemPreviewCard: React.FC<MenuItemPreviewCardProps> = ({
  ...
  return (
    ...
    <div style={{ transform: `scale(${scale})`, transformOrigin: "top center", padding: "16px", margin: "-16px" }}>
      {stateLabel && (
        <p className="text-center text-[10px] text-zinc-500 mb-2 font-medium uppercase tracking-widest">{stateLabel}</p>
      )}
