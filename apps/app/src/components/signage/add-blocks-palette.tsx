...
import { SignageLayoutConfig, ColumnLayoutSlide, SignageBlock } from "@soustools/api-types";
import { insertBlockIntoTree } from "./block-tree-utils";
import { LayoutGrid, AlignLeft, Image, List, AlertTriangle, CheckCircle2, Settings2, Image as ImageIcon, Video, GitCommit, ListTree } from "lucide-react";
...
const BLOCKS = [
  ...
  { id: "ImageBlock", label: "Static Image", type: "Content Block", icon: ImageIcon },
...
export const AddBlocksPalette: React.FC<AddBlocksPaletteProps> = ({
  ...
  const handleAddBlock = (blockType: string) => {
    setErrorMsg(null);
    if (!selectedBlockId || !selectedBlock) return setErrorMsg("Select a container first.");
    ...
    const newBlock: SignageBlock = blockType.endsWith("Block") 
      ? { id: `block-${Date.now()}`, type: blockType as any, blocks: [], cells: blockType === "GridBlock" ? [] : undefined } as any
        ...
        <div key={group}>
          ...
          <div className="grid grid-cols-2 gap-2">
            {BLOCKS.filter(b => b.type === group).map(block => {
              ...
              return (
                <button key={block.id} onClick={() => handleAddBlock(block.id)}
                  className={`flex flex-col items-center justify-center p-3 gap-2 bg-zinc-900 border rounded-xl transition-all cursor-pointer group ${
