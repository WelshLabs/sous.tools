...
export default function CatalogEditorPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12 animate-fadeIn relative">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-slate-100">POS Catalog Editor</h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Audit and edit properties of POS-synchronized menu items.</p>
      </div>
      ...
      {/* Catalog Grid */}
      {loading ? (
        <div className="text-center text-zinc-400 dark:text-zinc-500 py-12 text-sm">Downloading catalog metadata...</div>
      ) : filteredItems.length === 0 ? (
        <div className="text-center text-zinc-400 dark:text-zinc-500 py-12 text-sm">No items found matching the filter.</div>
      ) : (
          ...
          {filteredItems.map((item) => (
            <div 
              ...
              className={`glass-panel p-5 rounded-2xl border flex flex-col justify-between hover:border-white/15 transition-all relative ${
                item.is_sold_out ? "border-rose-500/20 bg-rose-950/5" : "border-black/5 dark:border-white/5"
              }`}
            >
              <div>
                <div className="w-full aspect-video rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-black/5 dark:border-white/5 flex items-center justify-center mb-4 text-zinc-700">
                  <Image className="w-8 h-8" />
                </div>
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-bold text-slate-200 text-sm truncate pr-2">{item.name}</h3>
                  <span className="text-xs font-bold text-sky-400 font-mono shrink-0">${item.price.toFixed(2)}</span>
                </div>
                <p className="text-zinc-400 dark:text-zinc-500 text-xs line-clamp-2 min-h-[32px]">{item.description || "No description provided."}</p>
              </div>
      ...
      {editingItem && (
        <div className="fixed inset-0 z-50 flex justify-end">
          <div className="absolute inset-0 bg-white/50 dark:bg-black/60 backdrop-blur-sm" onClick={() => setEditingItem(null)} />
          <div className="relative w-full max-w-md bg-zinc-50 dark:bg-zinc-950 border-l border-black/10 dark:border-white/10 shadow-2xl h-full flex flex-col animate-in slide-in-from-right duration-300">
            <div className="p-6 border-b border-black/5 dark:border-white/5 flex justify-between items-center bg-zinc-900/40">
            ...
            <form onSubmit={handleSave} className="flex-1 p-6 space-y-6 overflow-y-auto">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  required
                  className="w-full bg-zinc-100 dark:bg-zinc-900 border border-zinc-850 rounded-xl px-4 py-2.5 text-sm text-white focus:border-sky-500 outline-none"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea 
                  ...
                  rows={4}
                  className="w-full bg-zinc-100 dark:bg-zinc-900 border border-zinc-850 rounded-xl px-4 py-2.5 text-sm text-white focus:border-sky-500 outline-none resize-none"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    required
                    className="w-full bg-zinc-100 dark:bg-zinc-900 border border-zinc-850 rounded-xl pl-9 pr-4 py-2.5 text-sm text-white focus:border-sky-500 outline-none"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-black/5 dark:border-white/5 pt-6">
                <div>
                  <span className="text-xs font-bold text-slate-200 block">Inventory Status</span>
                  <span className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-0.5 block">Mark this item sold out globally</span>
                </div>
                <button
                  type="button"
