...
export class DietaryClassifierService {
  ...
  classifyRecipe(
    ...
    perServingMacros: Record<string, number>,
  ): Record<string, boolean> {
    this.logger.debug(
      `Classifying recipe with ${ingredients.length} ingredients`,
    );
    ...
    const hasTreeNuts = ingredients.some((i) =>
      i.allergens?.includes("tree_nuts"),
    );
    const hasPeanuts = ingredients.some((i) =>
      i.allergens?.includes("peanuts"),
    );
