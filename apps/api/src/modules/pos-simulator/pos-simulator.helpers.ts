...
} from "../integrations/drivers/square/square-sync.helper";
...
export async function resolveItemDetails(
  ...
  if (itemId) {
    ...
    if (data) {
      ...
      targetSquareId = data.external_id || undefined;
    }
  }
...
export async function handleSquareStockToggle(
  ...
  const { variationId, locationId } = await getVariationAndLocationId(
    accessToken,
    squareId,
  );
  ...
  if (!isSoldOut && unlimited) {
    ...
    if (!varRes.ok)
      throw new Error(`Fetch variation failed: ${await varRes.text()}`);
    ...
    if (version && variationObject) {
      ...
      if (!updateRes.ok)
        throw new Error(
          `Disable stock tracking failed: ${await updateRes.text()}`,
        );
    }
  } else {
    const varRes = await fetch(`${baseUrl}/v2/catalog/object/${variationId}`, {
    ...
    if (varRes.ok) {
      ...
      const trackInventory =
        variationObject?.item_variation_data?.track_inventory;
      if (version && !trackInventory && variationObject) {
        ...
        if (!enableRes.ok)
          throw new Error(
            `Enable stock tracking failed: ${await enableRes.text()}`,
          );
      }
    }
