...
export function ExplodedItemBlock({
  ...
  const containerClasses = [
    ...
    "rounded-2xl p-6 flex flex-col gap-6 text-slate-100 my-4",
    "st-exploded-item",
    className
  ].filter(Boolean).join(" ");
        ...
        <div className="flex flex-col gap-2 border-b border-white/10 pb-4">
          {(!hideTitle || !hidePrice) && (
            <div className="flex justify-between items-center flex-wrap gap-2">
              ...
              {!hidePrice && (
                <div className="text-[14px] font-medium text-slate-400 tracking-wider uppercase ml-auto">
          ...
          {!hideDescription && explodedItem.description && (
            <p className="text-sm text-slate-400 max-w-2xl">{explodedItem.description}</p>
          )}
        </div>
      )}
