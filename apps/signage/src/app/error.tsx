...
/**
 * ErrorBoundary is caught at the TV signage layout level.
 * It logs errors to the browser logger and displays a clean error fallback screen.
 */
export default function ErrorBoundary({ error, reset }: ErrorBoundaryProps) {
  useEffect(() => {
    logger.error({ err: error, digest: error.digest }, "Global signage layout error caught by boundary");
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-950 text-slate-50 p-4">
      <div className="glass-panel p-8 rounded-lg max-w-md border border-slate-800 text-center">
        <h2 className="text-xl font-semibold text-rose-500 mb-4">Display Error</h2>
        <p className="text-sm text-slate-400 mb-6">
          The signage stream encountered a rendering error. Auto-recovering...
        </p>
        <button
          onClick={() => reset()}
