...
@Command({
  name: "mcp:sync",
  aliases: ["mcp-sync"],
  description: "Symlink repository MCP config (mcp_config.json) to $HOME/.gemini/config",
})
export class McpSyncCommand extends CommandRunner {
  ...
  async run(): Promise<void> {
    ...
    try {
      if (fs.existsSync(targetConfig) || fs.lstatSync(targetConfig).isSymbolicLink()) {
    ...
    try {
      ...
      this.logger.error(`Failed to symlink MCP config: ${(err as Error).message}`);
    }
  }
}
