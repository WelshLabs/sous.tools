    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-50 flex items-center justify-center p-8 font-sans">
      <div className="max-w-2xl w-full bg-zinc-900/50 backdrop-blur-xl border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 border-b border-zinc-800 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-white">Sous Signage</h1>
            <p className="text-zinc-400 text-sm mt-1">Hardware Diagnostic & Setup</p>
          </div>
          <div className="flex gap-2">
            <div className={`h-2 w-12 rounded-full ${phase === 'wifi' ? 'bg-[#00FFFF]' : 'bg-zinc-800'}`} />
            <div className={`h-2 w-12 rounded-full ${phase === 'bootstrap' ? 'bg-[#00FFFF]' : 'bg-zinc-800'}`} />
            <div className={`h-2 w-12 rounded-full ${phase === 'pairing' || phase === 'done' ? 'bg-[#00FFFF]' : 'bg-zinc-800'}`} />
          </div>
        </div>
